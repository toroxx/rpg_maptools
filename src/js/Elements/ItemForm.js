...
const ItemTileSelector = (props) => {
    ...
    useEffect(() => {
        console.log(elements[txtbg]);
        if (elements[txtbg]) {
            let { path, filename, row_per_tile, tile_size, width, height } = elements[txtbg];
...
const ItemLayer = (props) => {
    ...
    useEffect(() => {
        ...
        setTileLayer(index, { bg: txtbg, id: txtid, zIndex: txtzIndex });
    }, [txtbg, txtid, txtzIndex]);
...
const ItemForm = (props) => {
    ...
    useEffect(() => {
        if (editItem != null) {
            ...
            if (editItem != "" && editItem != "-void-") {
                let tmp = (tiledata && tiledata[editItem] && tiledata[editItem]['layers']) || [];
                //console.log(editItem, tmp);
                setTileID(editItem);
                setTileLayers([...tmp]);
    ...
    const setTileLayer = (i, layer) => {
        let tmp = tile_layers; tmp[i] = layer; setTileLayers([...tmp]);
    };
                    ...
                    <tr>
                        ...
                        <td>
                            <div className="layers">
                                {tile_layers.map((tile_layer, i) => <ItemLayer key={i} index={i}
                                    elements={elements} layer={tile_layer} setTileLayer={setTileLayer} removeTileLayer={removeTileLayer} />)}
                            </div>
                        </td>
